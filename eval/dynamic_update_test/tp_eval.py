...
def remote_start_up_ok(ssh, num_of_rts):
  ...
  for x in range(1, num_of_rts+1):
    ...
    cmd="cat /home/net/nfa/eval/dynamic_update_test/rt"+str(x)+"_log.log"
    print cmd
...
def start_r2(ssh, options):
  ...
  success_flag = False;
  while(loop):
    cmd="sudo ~/nfa/eval/dynamic_update_test/clean_rt.sh"
    stdin,stdout,stderr =  ssh.exec_command(cmd);
    time.sleep(1)

    cmd="cd ~/nfa/eval/dynamic_update_test && sudo ./start_r2.sh " + str(options.r2_number) +" "+ str(options.service_chain)
    stdin,stdout,stderr =  ssh.exec_command(cmd);
    time.sleep(2)
...
def start_r3(ssh, options):
  ...
  while(loop):
    ...
    cmd="cd ~/nfa/eval/dynamic_update_test && sudo ./start_r3.sh " + str(options.r3_number) +" "+ str(options.service_chain)
    stdin,stdout,stderr =  ssh.exec_command(cmd);
    time.sleep(2)
...
def test():
  ...
  ssh_r2.exec_command('cd ~/nfa/eval/dynamic_update_test')
  ...
  ssh_r3.exec_command('cd ~/nfa/eval/dynamic_update_test')
