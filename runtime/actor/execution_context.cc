  ...
  if((replication_strategy==0)||(is_replica_alive == false)){
    ...
    if(set_up_entry == false){
      if(internal_pkt_counter % 3 ==0){
        send(replication_target_a, create_new_replica::value, this->id(), flow_identifier, service_chain_type_sig, 10);
      }
      servce_chain_process(input_pkt, from_p0);
      while(!output_queue->try_enqueue(input_pkt)){
      }
      ...
      if(internal_pkt_counter % 10 ==0){
        ...
        this->send(replication_target_a, from_p0, this->id(), true, scs_buf);
      }
      else{
...
void nf_execution_context::flow_finish_quit(migration_status status){
  if(status == migration_status::in_migration_source){
    ...
    send(worker_a, remove_from_active_nf_ecs::value, this->id());
  }
  else{
  }
  ...
  if(is_replica_alive){
    if(set_up_entry){
      send(replication_target_a, force_quit_atom::value, this->id());
    }
  ...
  send(nf_ec_timer_a, nf_ec_timer_quit::value);
  destroy(worker_a);
...
behavior nf_execution_context::migrate_flow_state(){
  ...
  this->request(migration_target_a, std::chrono::seconds(migration_timeout_ms), //10s long deadline
                try_migrate_flow_state::value, scs_buf).then(
    [=](nfactor_ok_atom){
      // at this time, the migration is successful
      // we can now turn to migration_finish_quit() state
      pending_transaction=false;
      if(cur_state == nf_ec_state::migrate_flow_state){
        //Attention: TODO: Please notify the worker packet processing loop.
...
behavior nf_execution_context::wait_flow_states(){
  ...
  return{
    ...
    [=](try_migrate_flow_state, vector<char>& scs_buf){
      service_chain_state scs;
      ...
      send(worker_a, remove_from_migration_target_nf_ecs::value, this->id(), this->migration_source_rt_id, false);
